...
The Model Selection Skill provides centralized model resolution for all five Teamsters agents (analyze, write, execute, validate, fix). Instead of hardcoding a single model in each agent's frontmatter, the orchestrator uses this skill to dynamically resolve which Claude model each agent should use at runtime.
...
2. Environment var    (TEAMSTERS_VALIDATE_MODEL=opus)
3. Config file        (.claude/teamsters-config.json model_overrides)
...
2. **Environment variable** (REQ-F-7): `TEAMSTERS_{AGENT}_MODEL` environment variable is set (e.g., `TEAMSTERS_VALIDATE_MODEL=opus`).
3. **Config file** (REQ-F-6): The `model_overrides` section in `{project_root}/.claude/teamsters-config.json` specifies a model for this agent.
...
Handles loading, validation, and persistence of the `.claude/teamsters-config.json` configuration file. Provides:
...
- Append-only Markdown metrics file at `{project_root}/.claude/teamsters-metrics.md` (REQ-F-12, REQ-F-15)
...
- `TEAMSTERS_DISABLE_METRICS` environment variable to opt out
...
```
Read environment variable: TEAMSTERS_{AGENT_NAME_UPPER}_MODEL
Example: For agent "validate", check TEAMSTERS_VALIDATE_MODEL
...
```
Load config from {project_root}/.claude/teamsters-config.json
...
Environment variables follow the naming convention `TEAMSTERS_{AGENT}_MODEL`:

```bash
# Per-agent model overrides
TEAMSTERS_ANALYZE_MODEL=sonnet
TEAMSTERS_WRITE_MODEL=sonnet
TEAMSTERS_EXECUTE_MODEL=haiku
TEAMSTERS_VALIDATE_MODEL=opus
TEAMSTERS_FIX_MODEL=opus
```
...
```
WARNING: Malformed config file at {project_root}/.claude/teamsters-config.json.
...
- [ ] Environment variable `TEAMSTERS_{AGENT}_MODEL` overrides config file (REQ-F-7)
...
- [ ] Metrics written to `.claude/teamsters-metrics.md` after each agent execution (REQ-F-12)
