...
**Purpose**: Non-blocking, append-only Markdown metrics logging for agent execution data, emitted to `{project_root}/.claude/teamsters-metrics.md`
...
```
{project_root}/.claude/teamsters-metrics.md
```

Where `{project_root}` is the detected project root directory (see `skills/project-detection/SKILL.md`).

The metrics file lives inside `.claude/` alongside the config file (`teamsters-config.json`) and state files. It should be added to `.gitignore` (contains execution data, not source).
...
FUNCTION emit_metrics(project_root, execution_data):
    ...
    TRY:
        ...
        metrics_path = "{project_root}/.claude/teamsters-metrics.md"
...
```
# Unix/macOS:
Use Bash tool: printf '%s\n' "{section}" >> "{project_root}/.claude/teamsters-metrics.md"

# Windows (PowerShell):
Use Bash tool: Add-Content -Path "{project_root}\.claude\teamsters-metrics.md" -Value "{section}"
```
...
existing = Read "{project_root}/.claude/teamsters-metrics.md"
    # On file-not-found: existing = ""

# Step 2: Write existing + new section
Use Write tool: write "{existing}\n{section}" to "{project_root}/.claude/teamsters-metrics.md"
```
...
**Log message** (warning):
```
WARNING: Failed to write metrics to {project_root}/.claude/teamsters-metrics.md: {error}
...
- [ ] **Append to file**: Use `Bash` tool or `Read`+`Write` tools to append to `teamsters-metrics.md`
...
- [ ] **Correct path**: Verify metrics file is at `{project_root}/.claude/teamsters-metrics.md`
...
After two test-loop executions, `{project_root}/.claude/teamsters-metrics.md` would contain:
